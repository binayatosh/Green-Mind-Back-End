Back-end Project Structure and File Overview
...
Flow of Operations
Initialization: __init__.py sets up the environment, logging, and configurations.
Data Loading: DataLoader loads and cleans the data.
Preprocessing: DataPreprocessor extracts features, scales the data, and splits it into training and testing sets.
Model Training: PowerConsumptionModel trains the model on the training data.
Model Evaluation: PowerConsumptionModel evaluates the model's performance on the test data.
Feature Importance: PowerConsumptionModel extracts feature importances.
Energy Analysis: EnergyAnalyzer analyzes energy consumption patterns and generates recommendations.
Visualization: EnergyVisualizer (or main.py directly) creates plots for prediction accuracy, feature importance, and hourly consumption.
API: api.py exposes the trained model for predictions via a web API.
Main Execution: main.py orchestrates all the above steps.
